...
void app_main(void)
{
    ...
    while (1) {
       ...
       //===================================================
        ...
        ESP_LOGI(TAG_CH[1][0], "raw  data: %d", adc_raw[0][1]);
        if (cali_enable) {
        ...
        ESP_LOGI(TAG_CH[2][0], "raw  data: %d", adc_raw[0][2]);
        if (cali_enable) {
        ...
        ESP_LOGI(TAG_CH[3][0], "raw  data: %d", adc_raw[0][3]);
        if (cali_enable) {
        ...
        ESP_LOGI(TAG_CH[4][0], "raw  data: %d", adc_raw[0][4]);
        if (cali_enable) {
            voltage = esp_adc_cal_raw_to_voltage(adc_raw[0][4], &adc1_chars);
            ESP_LOGI(TAG_CH[4][0], "cali data: %d mV", voltage);
        }
        vTaskDelay(pdMS_TO_TICKS(1000));
        ...
        ESP_LOGI(TAG_CH[5][0], "raw  data: %d", adc_raw[1][0]);
        if (cali_enable) {
            voltage = esp_adc_cal_raw_to_voltage(adc_raw[1][0], &adc2_chars);
            ESP_LOGI(TAG_CH[5][0], "cali data: %d mV", voltage);
        }
